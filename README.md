# Ранняя малоинвазивная фиксация из заднего доступа против других лечебных тактик при неосложненных переломах грудных и поясничных позвонков у живых взрослых: зонтичный систематический обзор литературы (1135 articles)
...
## Алгоритм скрининга по шагам

Начиная с 1-й записи в выборке библиографических записей:

* исходно запись не исключена — шаг 0 (при скрининге в PortoDB: крест не стоит),
* оценить текущую запись на соответствие шагу 1: если в тексте записи (при скрининге в PortoDB: только в заголовке записи) видим достаточные основания исключить по этому шагу, то исключить текущую запись (при скрининге в PortoDB: поставить крест) и перейти к следующей записи, иначе:
* оценить текущую запись на соответствие шагу 2: если в тексте записи (при скрининге в PortoDB: только в заголовке записи) видим достаточные основания исключить по этому шагу, то исключить текущую запись (при скрининге в PortoDB: поставить крест) и перейти к следующей записи, иначе:
* …
* оценить текущую запись на соответствие шагу 9: если в тексте записи (при скрининге в PortoDB: только в заголовке записи) видим достаточные основания исключить по этому шагу, то исключить текущую запись (при скрининге в PortoDB: поставить крест), перейти к следующей записи.
